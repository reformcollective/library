...
function isStyledCall(node) {
  return ts.isCallExpression(node) && ts.isIdentifier(node.expression) && node.expression.text === "styled"
}
...
function transformFile(sourceText, fileName) {
  ...
  for (const call of styledCalls) {
    ...
    for (const r of spreadRemovals.map((r) => ({ start: r.start - innerStart, end: r.end - innerStart }))) {
...
async function run() {
  const files = await globby(patterns.length ? patterns : defaultPatterns, {
    gitignore: true,
    ignore: ["**/node_modules/**", "**/.next/**", "**/dist/**", "**/app/library/**"],
    onlyFiles: true,
